...
describe("Booking API Tests", () => {
  ...
  it("Update Booking", async () => {
    ...
    const response = (await request(app).put(`/api/v1/bookings/${payLoad.bookingId}`).send(payLoad));
  ...
  it("Delete Booking", async () => {
    ...
    const response = (await request(app).delete(`/api/v1/bookings/${payLoad.bookingId}`));
    expect(response.status).toBe(200);
